...
function adminSwitchTab(tab) {
    ...
    if (tab === 'noticias') {
        ...
        document.getElementById('admin-content-noticias')?.classList.remove('hidden');
    ...
    else if (tab === 'noticias') { if(typeof adminLoadNoticias === 'function') adminLoadNoticias(); }
...
async function adminToggleAdmin(userId, makeAdmin, username) {
    const action = makeAdmin ? 'Promover a administrador' : 'Revocar administrador';
    if (!confirm('¿' + action + ' para "' + (username || 'este usuario') + '"?')) return;
    ...
    try {
        ...
        adminLoadUsuarios(searchTerm);
        showAdminToast('✅ Usuario "' + (username || 'usuario') + '" ' + (makeAdmin ? 'promovido a admin.' : 'revocado de admin.'));
    } catch (e) {
...
async function adminDeleteUser(userId, username) {
    if (!confirm('¿Eliminar al usuario "' + (username || 'este usuario') + '"?\n\nEsta acción no se puede deshacer.')) return;
    ...
    try {
        ...
        const searchTerm = document.getElementById('admin-user-search')?.value || '';
        adminLoadUsuarios(searchTerm);
        showAdminToast('🗑️ Usuario "' + (username || 'usuario') + '" eliminado.');
    } catch (e) {
